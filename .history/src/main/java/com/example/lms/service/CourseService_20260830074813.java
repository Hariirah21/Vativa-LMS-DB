...
public class CourseService {
    ...
    // SRS Field List - Course Status: fixed dropdown values
    private static final Set<String> ALLOWED_STATUS =
            Set.of("Draft", "Upcoming", "Active", "Archived");
    ...
    private String validateStatus(String rawStatus) {
        ...
        if (status.isEmpty()) {
            throw new ApiException("Course Status is required", HttpStatus.BAD_REQUEST);
        }
        boolean matched = ALLOWED_STATUS.stream().anyMatch(s -> s.equalsIgnoreCase(status));
        if (!matched) {
            throw new ApiException(
                    "Course Status must be one of: " + String.join(", ", ALLOWED_STATUS),
                    HttpStatus.BAD_REQUEST);
        }
        return status;
    }
