...
import com.example.lms.exception.CourseListLoadException;
import com.example.lms.exception.EnrollmentOperationException;
import com.example.lms.exception.EnrollmentOperationException.Operation;
...
public class CourseService {
    ...
    public CourseDto.CourseResponse createCourse(CourseDto.CourseRequest request, AuthPrincipal principal) {
        ...
        User instructor = getActiveInstructorOrThrow(request.getInstructorId());
    ...
    public List<CourseDto.CourseResponse> getAllCourses(String searchName, String status) {
        ...
        try {
            ...
            throw new CourseListLoadException(ex);
        }
    ...
    @Transactional(readOnly = true)
    public List<CourseDto.InstructorOption> getActiveInstructors() {
        return userRepository
                .findByRoleIgnoreCaseAndActiveTrueOrderByFirstNameAscLastNameAsc(INSTRUCTOR_ROLE)
                .stream()
                .map(user -> CourseDto.InstructorOption.builder()
                        .id(user.getId())
    ...
    @Transactional(readOnly = true)
    public List<CourseDto.EnrolledCourseResponse> getEnrolledCourseList(Long courseId) {
        try {
            getCourseOrThrow(courseId);
            return courseEnrollmentRepository.findByCourseId(courseId).stream()
                    .map(this::toEnrolledCourseResponse)
                    .toList();
        } catch (DataAccessException ex) {
            throw new EnrollmentOperationException(Operation.LIST_LOADING, ex);
        }
    }

    @Transactional
    public List<CourseDto.EnrolledCourseResponse> enrollUsers(CourseDto.EnrollUsersRequest request) {
        try {
            Long courseId = request.getCourseId();
            String clientRequestId = request.getClientRequestId().trim();
            courseRepository.findByIdForEnrollment(courseId)
                    .orElseThrow(() -> new ApiException(
                            "Course not found with id: " + courseId,
                            HttpStatus.NOT_FOUND));

            List<CourseEnrollmentEntity> previousResult =
                    courseEnrollmentRepository.findByCourseIdAndClientRequestId(
                            courseId, clientRequestId);
            ...
            List<CourseEnrollmentEntity> enrollments = request.getUserIds().stream()
                    .distinct()
                    .filter(userId -> !courseEnrollmentRepository
                            .isUserEnrolledInCourse(userId, courseId))
                    .map(userId -> CourseEnrollmentEntity.builder()
                            .courseId(courseId)
                            .userId(userId)
            ...
            return courseEnrollmentRepository.saveAllAndFlush(enrollments).stream()
                    .map(this::toEnrolledCourseResponse)
                    .toList();
        } catch (DataAccessException ex) {
            throw new EnrollmentOperationException(Operation.ENROLLMENT, ex);
        }
    }

    @Transactional
    public void unenrollUsers(CourseDto.UnenrollUsersRequest request) {
        try {
            Long courseId = request.getCourseId();
            getCourseOrThrow(courseId);

            List<Long> enrollmentIds = request.getEnrollmentIds().stream()
                    .distinct()
                    .toList();
            List<CourseEnrollmentEntity> enrollments =
                    courseEnrollmentRepository.findAllById(enrollmentIds);

            boolean allSelectionsBelongToCourse = enrollments.size() == enrollmentIds.size()
                    && enrollments.stream().allMatch(enrollment -> courseId.equals(enrollment.getCourseId()));
            if (!allSelectionsBelongToCourse) {
                throw new ApiException(
                        "One or more selected enrollments do not belong to the selected course.",
            ...
            throw new EnrollmentOperationException(Operation.UNENROLLMENT, ex);
        }
    }
    ...
    private User getActiveInstructorOrThrow(Long instructorId) {
        if (instructorId == null) {
            throw new ApiException("Instructor selection is required", HttpStatus.BAD_REQUEST);
        }
        return userRepository
                .findByIdAndRoleIgnoreCaseAndActiveTrue(instructorId, INSTRUCTOR_ROLE)
                .orElseThrow(() -> new ApiException(
                        "Please select a valid active instructor.",
                        HttpStatus.BAD_REQUEST));
    }
    ...
    private CourseDto.EnrolledCourseResponse toEnrolledCourseResponse(CourseEnrollmentEntity enrollment) {
        return CourseDto.EnrolledCourseResponse.builder()
