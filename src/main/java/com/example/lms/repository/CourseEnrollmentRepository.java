...
public interface CourseEnrollmentRepository extends JpaRepository<CourseEnrollmentEntity, Long> {
    ...
    // Enrolled Course List for a course, including all persisted learner tracking fields
    List<CourseEnrollmentEntity> findByCourseId(Long courseId);

    // Previously created rows for an idempotent Enroll User submission
    List<CourseEnrollmentEntity> findByCourseIdAndClientRequestId(
            Long courseId, String clientRequestId);
